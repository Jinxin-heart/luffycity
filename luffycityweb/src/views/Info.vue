  ...
  <div class="detail">
    ...
    <div class="main">
      <div class="course-info">
        <div class="wrap-left">
          <!-- 课程封面或封面商品 -->
          <AliPlayerV3 ref="player" class="h-64 md:h-96 w-full rounded-lg" style="height: 100%; width: 100%;"
                       :options="options" @play="onPlay($event)" @pause="onPause($event)" @playing="onPlaying($event)"
                       v-if="course.info.course_video"/>
          <img :src="course.info.course_cover" style="width: 100%;"
               alt="" v-else>
        </div>
        <div class="wrap-right">
          <h3 class="course-name">{{ course.info.name }}</h3>
          <p class="data">
            {{ course.info.students }}人在学 课程总时长：{{ course.info.pub_lessons }}课时/{{
              course.info.lessons
            }}课时 难度：{{ course.info.get_level_display }}
          </p>
          <div class="sale-time">
            ...
            <p class="expire" v-if="course.info.discount.expire > 0">距离结束：仅剩
              ...
              {{ fill0(parseInt(course.info.discount.expire / 60 % 60)) }}分 <span class="second">{{
                  fill0(parseInt(course.info.discount.expire % 60))
                }}</span> 秒</p>
          </div>
      ...
      <div class="course-content">
        <div class="course-tab-list">
          <div class="tab-item" v-if="course.tabIndex === 1">
            <p><img alt="" src="//hcdn2.luffycity.com/media/frontend/activity/详情页_01.png"></p>
          </div>
          <div class="tab-item" v-if="course.tabIndex === 2">
            <div class="tab-item-title">
              <p class="chapter">课程章节</p>
              <p class="chapter-length">共 {{ course.chapter_list.length }} 章 {{ totalLessons }} 个课时</p>
            </div>
            <div class="chapter-item" v-for="(chapter, chapterIndex) in course.chapter_list"
                 :key="chapter.id">
              <!-- 章节标题 -->
              ...
              <ul class="lesson-list">
                <li class="lesson-item" v-for="(lesson, lessonIndex) in chapter.get_lesson_list"
                    :key="lesson.id">
                  <p class="name">
                    <span class="index">{{ chapterIndex + 1 }}-{{ lessonIndex + 1 }}</span>
                    {{ lesson.name }}
                    <span class="free" v-if="lesson.free_trail">免费</span>
                  </p>
                  <p class="time">
                    {{ lesson.duration }}
                    <img src="../assets/chapter-player.svg">
                  </p>
                  <button class="try">
                    {{ lesson.free_trail ? "立即试学" : "立即购买" }}
                  </button>
                </li>
              </ul>
            </div>
            ...
            <div class="teacher-content">
              <div class="cont1">
                ...
                <div class="name">
                  ...
                  <p class="teacher-title">{{ course.info.teacher.get_role_display }}</p>
                </div>
              </div>
